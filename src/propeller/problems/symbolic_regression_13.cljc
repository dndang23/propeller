(ns propeller.problems.symbolic_regression_13
...
(defn- target-function
  "Target function: f(x) = (1 + x^3)^4 + 1"
  [x]
  (inc (* (inc (* x x x)) (inc (* x x x)) (inc (* x x x)) (inc (* x x x)))))
...
(defn multiple-evaluation-function
  [argmap data individual]
  (loop [i 0 limit 20 min_behaviors_list '() min_error_list '() min_total_error 2147483647 min_program '() min-program-boolean-plushy '()]
    (if (= i limit)
      (assoc individual
    ...
    (if (= num_tries 1)
      ...
      (do
        ...
        (if (= type "probabilistic-epsilon-lexicase")
          ...
          (if (= type "default-epsilon-lexicase-increased-generations")
            (let [output   (gp/gp
                             (merge
                               {:instructions             instructions
